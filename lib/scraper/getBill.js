...
var getBill = function(billName, callback, options){
  ...
  if(typeof clean != "boolean") throw new TypeError("'options.clean' must be a boolean"); //TODO: add an error handler for session number
  ...
  if(!/^BILLS-11[3-5](sres|sjres|sconres|s|hres|hr|hjres|hconres)[0-9]+[a-z]{2}$/i.test(billName)) throw new TypeError("'billName' needs to be a valid bill name");

  var requestUrl = "https://www.gpo.gov/fdsys/bulkdata/BILLS/" + billName.replace("BILLS-", "").replace(/(sres|sjres|sconres|s|hres|hr|hjres|hconres).+$/g, "") + "/" + session + "/" + billName.replace(/BILLS-11[3-5]/, "").replace(/[0-9]+[a-z]{2}$/g, "") + "/" + billName + ".xml";
  ...
  request(requestUrl, function (err, res, xml){ //Access the XML file from the GPO
    ...
    xml = xml.replace(/<\/?(quote|short-title)>/gi, "");
