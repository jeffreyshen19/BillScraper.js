 ...
 * @param {boolean} options.clean - Enable / disable automatic cleaning of bill (default: false)
...
var getBill = function(billName, callback, options){
  options = options || {};
  var clean = ("clean" in options) ? options.clean : false;
  ...
  if(!/^BILLS-115(sres|sjres|sconres|s|hres|hr|hjres|hconres)[0-9]+[a-z]{2}$/i.test(billName)) throw new TypeError("'billName' needs to be a valid bill name");

  var requestUrl = "https://www.gpo.gov/fdsys/bulkdata/BILLS/115/" + session + "/" + billName.replace("BILLS-115", "").replace(/[0-9]+[a-z]{2}$/g, "") + "/" + billName + ".xml";
