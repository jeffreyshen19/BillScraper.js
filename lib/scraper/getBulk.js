...
var getBulk = function(billType, callback, options){
  ...
  if(typeof congress != "number" || congress < 113 || congress > 115) throw new TypeError("'options.congress' must be a number between 113 and 115", "getBulk.js", 28);
  if(typeof session != "number" || (session != 1 && session != 2)) throw new TypeError("'options.session' must be either 1 or 2", "getBulk.js", 29);

  if(typeof billType != "string") throw new TypeError("'billType' needs to be a String", "getBulk.js", 31);
  if(callback == null) throw new TypeError("A callback needs to be included", "getBulk.js", 32);

  billType = billType.toLowerCase();
  if(!validBillTypes.includes(billType)) throw new TypeError("'billType' needs to be a valid bill type", "getBulk.js", 35);
