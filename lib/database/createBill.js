...
var createBill = function(bill, options){
  ...
  if(typeof session != "number" || (session != 1 && session != 2)) throw new TypeError("'options.session' must be either 1 or 2", "createBill.js", 23);
  if(typeof bill != "string") throw new TypeError("'bill' needs to be a string", "createBill.js", 24);
  if(!/^BILLS-11[3-5](s|hr)[0-9]+[a-z0-9]{2,}$/i.test(bill)) throw new TypeError("'bill' needs to be a valid bill name", "createBill.js", 25);
          ...
          printBill(res, function(parsedResult){
            ...
            tagBill(parsedResult, function(tags){
              ...
              getBillStatus(billStatusName, function(res){
                ...
                res.cosponsors.forEach(function(sponsor){
                  if(sponsor.isOriginalCosponsor.toLowerCase() === "true") newBill.sponsors.push(sponsor.fullName);
                });

                newBill.save(function(err){
                  if(err) throw Error("Unable to save bill to database", "createBill.js", 65);
